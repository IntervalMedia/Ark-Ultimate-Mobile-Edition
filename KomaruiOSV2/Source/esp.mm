...
//Offsets
// ----------------------------------------------
constexpr uintptr_t OFFSET_GWORLD = 0x04d5b510; //GWorld
constexpr uintptr_t OFFSET_LEVEL = 0x1D0; //Ulevel /
constexpr uintptr_t OFFSET_ACTOR_ARRAY = 0x98; //ActorArray (never changes)
constexpr uintptr_t OFFSET_ACTOR_COUNT = 0xA0; //ActorArray_c (never changes)
constexpr uintptr_t OFFSET_GAME_INSTANCE = 0x320; //struct UGameInstance* OwningGameInstance; /
constexpr uintptr_t OFFSET_LOCAL_PLAYERS = 0x38; //LocalPlayers /
constexpr uintptr_t OFFSET_PLAYER_CONTROLLER = 0x30; //LocalPlayerController /
constexpr uintptr_t OFFSET_CAMERA_MANAGER = 0x480; //PlayerCameraManager /
constexpr uintptr_t OFFSET_VIEW_TARGET = 0x1150; //struct FTViewTarget* ViewTarget; /
constexpr uintptr_t OFFSET_POV = 0x10; //struct FMinimalViewInfo POV;
// ----------------------------------------------
//Offsets END
// ----------------------------------------------
...
// actor ID filtering
std::unordered_map<int, std::string> actorIDMap = {
    {3312, "Player"},
    {552, "S_WoodBox"},
    {4040, "Bed"}
};
...
void ESP::RenderESP(ImDrawList* drawList) {
    ...
    for (const auto& actor : DrawCache) {
        ...
        // Check if the actor ID exists in the unordered map
        std::string actorLabel;
        auto it = actorIDMap.find(actor.id); // Look for the actor ID in the map
        if (it != actorIDMap.end()) {
            actorLabel = it->second; // Custom label found
        } else {
            actorLabel = "ID: " + std::to_string(actor.id); // Default if no actorID found
        }
...
//ESP Box
void ESP::RenderESP2DBox(ImDrawList* drawList) {
    if (!Variables.Draw2DBoxes) return; // exit if bool is disabled to prevent crashes

    for (const auto& actor : DrawCache) {
        if (actor.id == 3312) { // Player ID
            FVector screenData = W2S(actor.W2SPos, camInfo, camInfo.Rotation);
            FVector extraScreenData = W2S(FVector{actor.W2SPos.X, actor.W2SPos.Y, actor.W2SPos.Z - 80.0f}, camInfo, camInfo.Rotation);
            ...
            // box corners
            ImVec2 topLeft(screenData.X - width / 2, screenData.Y - height);
            ImVec2 bottomRight(screenData.X + width / 2, screenData.Y);
