...
void* BasicHacks::HacksThread(void* arg)
{
    ...
    while(true)
    {
        ...
        uintptr_t GWorld                  = *(uintptr_t*)(BaseAddr              + 0x0506EEC0);    if (!IsValidPointer(GWorld))                     continue;
        ...
        uintptr_t LocalPlayer             = *(uintptr_t*)(LocalPlayers);                          if (!IsValidPointer(LocalPlayer))                continue; //(first instance in the array (0) is LocalPlayer)
        ...
        *(float*)(PlayerCameraManager + 0x33a0) = Variables.FOV; //NormalFOV
